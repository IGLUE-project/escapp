...
module.exports = {
    async up(queryInterface, Sequelize) {
        ...
        let [attachments] = await queryInterface.sequelize.query(`
        ...
        for (const thumbnail of attachments) {
          ...
          //Download file
          try {
              let downloadFileName = path.basename(new URL(thumbnail.url).pathname);
              let downloadFilePathFull = destDownloadsFolder + "/" + downloadFileName;
              ...
              let thumbnailData = await getDataForFile(downloadFilePathFull);
              let thumbnailFileId = "cloudinaryt-" + thumbnail.id + "-" + downloadFileName;
              // let assetFilePath = "/uploads/" + assetFileId;
              let thumbnailFilePathFull = destThumbnailsFolder + "/" + thumbnailFileId;
